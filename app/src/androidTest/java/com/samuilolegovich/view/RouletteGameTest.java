...
/**
 * Smoke-тест для RouletteGame. IS_REAL_GAME_MODE = false переводит
...
public class RouletteGameTest {
    ...
    public void app_launchesWithoutCrash() {
        try (ActivityScenario<RouletteGame> scenario = ActivityScenario.launch(RouletteGame.class)) {
            scenario.onActivity(activity -> assertNotNull(activity));
        }
    }

    @Test
    public void screen_showsBetFieldAndSpinButton() {
        try (ActivityScenario<RouletteGame> scenario = ActivityScenario.launch(RouletteGame.class)) {
            onView(withId(R.id.bet_field)).perform(scrollTo()).check(matches(isDisplayed()));
            onView(withId(R.id.btn_spin_roulette)).perform(scrollTo()).check(matches(isDisplayed()));
        }
    }
}
