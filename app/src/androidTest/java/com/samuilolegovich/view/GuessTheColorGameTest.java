...
/**
 * Smoke-тест для GuessTheColorGame. IS_REAL_GAME_MODE = false переводит
...
public class GuessTheColorGameTest {
    ...
    public void app_launchesWithoutCrash() {
        try (ActivityScenario<GuessTheColorGame> scenario = ActivityScenario.launch(GuessTheColorGame.class)) {
            scenario.onActivity(activity -> assertNotNull(activity));
        }
    }

    @Test
    public void screen_showsBetFieldAndColors() {
        try (ActivityScenario<GuessTheColorGame> scenario = ActivityScenario.launch(GuessTheColorGame.class)) {
            onView(withId(R.id.bet_field)).check(matches(isDisplayed()));
            onView(withId(R.id.color_red)).check(matches(isDisplayed()));
            onView(withId(R.id.color_black)).check(matches(isDisplayed()));
        }
    }
}
