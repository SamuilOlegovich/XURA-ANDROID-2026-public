...
/**
 * Smoke-тест для TransactionHistory. MainActivity.IS_REAL_GAME_MODE = false
 * заставляет экран использовать тестовые данные истории вместо реального
 * обращения к XRPL-сети.
 */
...
public class TransactionHistoryTest {
    ...
    public void app_launchesWithoutCrash() {
        try (ActivityScenario<TransactionHistory> scenario = ActivityScenario.launch(TransactionHistory.class)) {
    ...
    public void screen_showsTitle() {
        try (ActivityScenario<TransactionHistory> scenario = ActivityScenario.launch(TransactionHistory.class)) {
            onView(withId(R.id.transaction_history_text_view)).check(matches(isDisplayed()));
        }
    }
}
