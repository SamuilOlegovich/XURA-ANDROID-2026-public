...
готово, clipboard-защита — готово, ProGuard/Network Security Config —
в работе) описаны отдельно по ходу работы и не входят в этот документ.

## Отложено: Play Integrity API (п.8 общего плана)

Play Integrity API проверяет на сервере, что вердикт целостности устройства
и подлинности установки (APK подписан и получен из Google Play, устройство
не модифицировано) совпадает с ожидаемым. Технически это требует:

1. Клиент получает integrity token через `IntegrityManager` (Google Play
   Integrity SDK) и отправляет его на **свой backend**.
2. **Backend** расшифровывает/проверяет токен через Google Play Integrity
   API (decrypt + verify подписанный JWT) и возвращает клиенту решение —
   пускать или блокировать.

Без доступа к backend-части реализовать пункт целиком нельзя — клиентский
токен сам по себе ничего не проверяет, доверять его расшифровке на
клиенте бессмысленно (его легко подделать/обойти на скомпрометированном
устройстве). Поэтому пункт 8 отложен до появления доступа к серверной
части проекта. Когда доступ появится — нужно:

- завести проект в Google Cloud Console, включить Play Integrity API;
- на backend добавить эндпоинт декодирования токена через
  `playintegrity.googleapis.com/v1/{package}:decodeIntegrityToken`;
- на клиенте дёргать `IntegrityManager.requestIntegrityToken(...)` перед
  чувствительными операциями (например, перед отправкой платежа) и
  блокировать действие при негативном вердикте с backend.
